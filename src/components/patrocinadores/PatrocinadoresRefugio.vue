...
const props = defineProps<{ patrocinadores: IPatrocinador[]}>()

</script>

<template>
    <div>
        <DataTable :value="patrocinadores">
            <template #empty>
            ...
            <template #header>
                <div class="flex flex-row bg-surface-50 gap-3">
                    ...
                    <div class="my-auto flex w-full justify-between items-center">
                        ...
                        <Button icon="pi pi-plus" icon-pos="left" label="Patrocinador" 
                        @click="() => modalesStore.abrir('nuevoPatrocinador')"></Button>
                    </div>
                </div>
            </template>
            <Column header="Nombre" field="nombre">
            </Column>
            <Column header="Datos de contacto" field="contacto">
            </Column>
            ...
            <Column>
                <template #body="{data}">
                    <Button icon="pi pi-eye" @click="() => router.push(`/refugio/patrocinador/${data.id_patrocinador}`)"></Button>
                </template>
            </Column>
        </DataTable>
    </div>
