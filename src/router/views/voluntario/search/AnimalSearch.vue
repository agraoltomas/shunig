...
import { type IModalesContext, useModalStore } from '@/stores/modales.ts'
...
const props = defineProps<{action: (Extract<keyof IModalesContext,'adopcion'|'nuevoTransito'>),filtros?: { estado?: ('transito'|'refugio')[]}}>()
...
onMounted(async () => {
    ...
    if (route.query.m) {
        await loadMascota()
        if (!mascota.value) {
            toast.add({ severity: 'info', detail: "El animal que busca no existe"})
        } else {
            modal.abrir('mostrarAnimal', mascota.value)
        }
    } else {
                ...
                <template #default="{ item}">
                    <div class="flex flex-col gap-3 border-surface-200 w-full">
                        ...
                        <div class="flex flex-row gap-3 pb-3 justify-between">
                            <div class="flex flex-row gap-3 max-w-80 text-wrap wrap-normal">
    ...
    <Modal nombre="mostrarAnimal" class="min-w-[35em] m-auto!" modal title="">
        <template #default="{closeFn, context}">
            ...
            <div class="flex flex-row p-3 justify-end">
                <slot name="actionButton" v-bind="{animal: context.mostrarAnimal, accion: () => modal.abrir(action, { mascota: context.mostrarAnimal, usuario: authStore.user })}">
                </slot>
<!--                <Button outlined class="mx-auto!" label="Adoptar"-->
<!--                               @click.capture=" () => {-->
<!--                                       console.log('aca');-->
